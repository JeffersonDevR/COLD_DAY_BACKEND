...
public interface SpringDataOtRepository extends JpaRepository<OtJpaEntity, UUID> {
     ...
     *
     * <p>The auxiliar count rides this single conditional {@code UPDATE} as one
     * more {@code SET} item (design AD2): with {@code clearAutomatically=true} a
     * second write on the {@code @Version} aggregate would race the cleared
     * persistence context, so the winner's only write must already carry it.
     */
    @Modifying(clearAutomatically = true, flushAutomatically = true)
    @Query("UPDATE OtJpaEntity o SET o.estado = :destino, o.tecnicoId = :tecnicoId, "
            + "o.asignadaEn = :ahora, o.radioKm = :radioKm, "
            + "o.auxiliaresRequeridos = :auxiliaresRequeridos "
    ...
    int asignarSiDisponible(@Param("id") UUID id, @Param("tecnicoId") UUID tecnicoId,
            ...
            @Param("auxiliaresRequeridos") int auxiliaresRequeridos,
