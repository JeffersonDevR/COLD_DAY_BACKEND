...
public class TecnicoJpaEntity {
    ...
    public Tecnico toDomain() {
        Tecnico target = new Tecnico();
        target.setId(id);
        target.setNumeroIdentificacion(numeroIdentificacion);
        target.setNombres(nombres);
        target.setApellidos(apellidos);
        target.setTelefono(telefono);
        target.setEmail(email);
        target.setFotoUrl(fotoUrl);
        target.setCategoriasServicio(categoriasServicio == null ? new HashSet<>() : new HashSet<>(categoriasServicio));
        target.setEstadoOperativo(estadoOperativo);
        target.setCertificaciones(certificaciones == null ? new HashSet<>() : new HashSet<>(certificaciones));
